        ...
        if len(self.units(NEXUS)) * 16 > len(self.units(PROBE)):
            if len(self.units(PROBE)) < self.MAX_WORKERS:
                for nexus in self.units(NEXUS).ready.noqueue:
                    if self.can_afford(PROBE) and self.supply_left > 5 and self.units(PROBE).amount < self.MAX_WORKERS:
                        await self.do(nexus.train(PROBE))
                        time.sleep(0.01)

    async def build_supply(self):
        if not self.already_pending(PYLON) and self.supply_left < 5:
            ...
            if self.units(PYLON).ready.exists:
                ...
                if self.units(CYBERNETICSCORE).ready.exists:
                    if len(self.units(ROBOTICSFACILITY)) < 1:
                        if self.can_afford(ROBOTICSFACILITY) and not self.already_pending(STARGATE):
...
run_game(maps.get('AbyssalReefLE'), [
    Bot(Race.Protoss, ProtBot()),
    Computer(Race.Random, difficulty=Difficulty.Hard)
], realtime=False)
